<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <!-- Satuan Kerja -->
        <div class="mt-4">
            <x-input-label for="satuan_kerja" :value="__('Satuan Kerja')" />
            <x-text-input id="satuan_kerja" class="block mt-1 w-full" type="text" name="satuan_kerja" :value="old('satuan_kerja')" required />
            <x-input-error :messages="$errors->get('satuan_kerja')" class="mt-2" />
        </div>

        <!-- Role (Optional, but assuming 'operator' by default) -->
        <div class="mt-4">
            <x-input-label for="role" :value="__('Role')" />
            <select id="role" name="role" class="block mt-1 w-full">
                <option value="operator" {{ old('role') == 'operator' ? 'selected' : '' }}>{{ __('Operator') }}</option>
                <option value="admin_bnn" {{ old('role') == 'admin_bnn' ? 'selected' : '' }}>{{ __('Admin BNN') }}</option>
            </select>
